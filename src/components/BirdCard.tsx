...
export default function BirdCard({ bird, showPlayButton }: BirdCardProps) {
  ...
  const handlePlayClick = (e: React.MouseEvent) => {
    ...
    if (!audioRef.current) {
      ...
      const audioUrl = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/ghvogelstationfiles/today/${bird.scientific_name.replace(/ /g, '_')}.mp3`
      ...
      // Setup AudioContext and gain node for volume boost
      ...
      if (AudioContextClass) {
        try {
          ...
          gainNode.gain.value = 5.0 // Increase volume by 5x
          source.connect(gainNode)
          gainNode.connect(audioCtxRef.current.destination)
        } catch (e) {
          console.error("Audio volume boost failed:", e)
        }
      }
    }
